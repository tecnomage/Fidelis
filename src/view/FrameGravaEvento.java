...
import controller.MonitoraEspacoThread;
import controller.Sessao;
import controller.ThreadMonitorMicrofone;
import controller.UpperField;
import de.humatic.dsj.DSFiltergraph;

@SuppressWarnings("serial")
public class FrameGravaEvento extends JPanel {
	...
	public FrameGravaEvento(String strUsuarioLogado) {		
		...
		JLabel lb3 = new JLabel("Nome do Evento:");
		lb3.setBounds(MARGEM_LABEL, Y+30, 200, 20);
		...
		tfNumProcesso.setText("0000000-00.0000.0.00.0000");
		...
		tfJuiz.setBounds(MARGEM_LABEL, Y+60, 333, 20);
		if (Sessao.numProcesso != null) {
		...
		pTemas = new PanelTreeTemasEQualif(usuarioLogado, "", "TEMASE");
		pTemas.setLocation(370, 1);
		this.add(pTemas);
		
		pQualifica = new PanelTreeTemasEQualif(usuarioLogado, "", "QUALIFICACOESE");
		...
		btMarcarVideo.setBounds(189, 5, 140, 25);
	...
	private void btIncluirEvent(java.awt.event.ActionEvent evt) {
		...
		if (Sessao.nomeDispositivoVideo.equals("nenhum")) {
			JOptionPane.showMessageDialog(null, "Nenhum dispositivo de vídeo foi selecionado. Não é possível prosseguir com a validação.","GRAVAÇÃO DE EVENTO", JOptionPane.ERROR_MESSAGE);
			return;
		}

		if (Sessao.nomeDispositivoAudio.equals("nenhum")) {
			JOptionPane.showMessageDialog(null, "Nenhum dispositivo de áudio foi selecionado. Não é possível prosseguir com a validação.","GRAVAÇÃO DE EVENTO", JOptionPane.ERROR_MESSAGE);
			return;
		}
		
		tfNumProcesso.setText(tfNumProcesso.getText().trim());
		if (!validaNumProcesso(tfNumProcesso.getText())) {
			JOptionPane.showMessageDialog(null, "Número de Processo inválido!", "GRAVAÇÃO DE EVENTO", JOptionPane.WARNING_MESSAGE);
		}
		else {
			if (tfJuiz.getText().equals("") || tfJuiz.getText() == null) {
				JOptionPane.showMessageDialog(null, "Por favor informe o nome do Evento!", "GRAVAÇÃO DE EVENTO", JOptionPane.WARNING_MESSAGE);
			}
			else {
				...
				//	JOptionPane.showMessageDialog(null, "Este processo já foi CONCLUÍDO anteriormente.\nNão é possível gravar mais vídeos para este processo.", "GRAVAÇÃO DE EVENTO", JOptionPane.ERROR_MESSAGE);
				//}
				//else {				
					pTemas.carregaTemas(tfNumProcesso.getText());			
					pQualifica.carregaQualifica(tfNumProcesso.getText(), null);				
					tfNumProcesso.setEditable(false);
					cbTipAudiencia.setEnabled(false);
					tfJuiz.setEditable(false);
	...
	private void btMarcarVideoEvent(java.awt.event.ActionEvent evt) {		
    	...
    	String strMsg = "Dados da Marcação do Video:\n";
    	strMsg += "\nTema: " + strTemas[0];
    	strMsg += "\nSubTema: " + strTemas[1];
    	strMsg += "\n\nDepoente: " + strQualifica[1];
    	strMsg += "\nQualificação: " + strQualifica[0];
    	strMsg += "\n\nTempo no Vídeo: " + strTempo;
    	strMsg += "\n\nDeseja efetivar esta marcação?";
    	
    	if (!strTemas[0].equals("") || !strTemas[1].equals("") || !strQualifica[0].equals("") || !strQualifica[1].equals("")) {
    		
        	Object[] options = { "Sim", "Não" };
        	int resposta = JOptionPane.showOptionDialog(null, strMsg, "Marcação do Vídeo", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null, options, options[0]);
	...
	private void btConcluiEvent(java.awt.event.ActionEvent evt) {
		...
		if (dealer.mudaStatus("VINCULADO")) {
			logInfo.grava("fidelis.view.FrameGravaAudiencia.btConcluiEvent()", usuarioLogado, "002-Gravação do Evento (" + tfJuiz.getText() + ") foi CONCLUÍDA!");
		}

		setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));		
		JOptionPane.showMessageDialog(null, "Gravação do Evento Finalizada!", "GRAVAÇÃO DE EVENTO", JOptionPane.INFORMATION_MESSAGE);
		jifPai.dispose();
	}
	...
	private void btInterrompeEvent(java.awt.event.ActionEvent evt) {
		...
		if (dealer.mudaStatus((String) cbMotivoFinalizacao.getSelectedItem())) {
			logInfo.grava("fidelis.view.FrameGravaAudiencia.btInterrompeEvent()", usuarioLogado, "Gravação do Evento (" + tfJuiz.getText() + ") foi INTERROMPIDA! Motivo: " + (String) cbMotivoFinalizacao.getSelectedItem());
		}
	...
	public void interrompePorErroSincronia() {
		...
		logInfo.grava("fidelis.view.FrameGravaEvento.interrompePorErroSincronia()", usuarioLogado, "Gravação do Evento (" + tfJuiz.getText() + ") foi INTERROMPIDA! Motivo: ERRO DE SINCRONIA!");
		setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
		JOptionPane.showMessageDialog(null, "Gravação do Evento Finalizada!", "GRAVAÇÃO DE EVENTO", JOptionPane.INFORMATION_MESSAGE);
		jifPai.dispose();
	}
	...
	public void gravaVideo() {
		...
		try {
			...
			setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
			JOptionPane.showMessageDialog(null, "O Processo de gravação de vídeo NÃO pôde ser inicializado!", "GRAVAÇÃO DE EVENTO", JOptionPane.ERROR_MESSAGE);
			gravaVideoBackOut();
		}
	...
	public void gravaAudio() {
		...
		try {
			...
			setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
			JOptionPane.showMessageDialog(null, "O Processo de gravação de áudio NÃO pôde ser inicializado!", "GRAVAÇÃO DE EVENTO", JOptionPane.ERROR_MESSAGE);
			gravaVideoBackOut();
		}		
	}
